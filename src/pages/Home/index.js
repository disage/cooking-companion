            ...
            const unsub = onSnapshot(q, (querySnapshot) => {
                if (querySnapshot.docs[0]) {
                    ...
                    const productNames = querySnapshot.docs[0].data().products
                        ? querySnapshot.docs[0].data().products
            ...
            <h4>We suggest you cook:</h4>
            {
                (!isLoading && suggesedDish !== []) ?
                    suggesedDish.map((item, index) => (
                        <div key={index} className="suggesedDish">
                            ...
                            <div className="actions">
                                {suggesedDish.length > 1 && <Button type="button" text="Remove" btnStyle="secondary" onButtonClick={() => removeSuggesedDish(index)} />}
                    ...
                    <div className="suggesedDish">
                        <h4>{userProducts?.products?.length === 0 ? 'You should add some products' : 'We are generate your dish ...'}</h4>
                    </div>
            }
            {isLoadingNewDish && <div className="suggesedDish">
                <h4>{'We are generate your dish ...'}</h4>
            </div>}
            {(!isLoadingNewDish && !isLoading) && <Button type="button" text="Generate New Dish" btnStyle="secondary" onButtonClick={generateNewDish} />}
            <Form actionType="add" userProducts={userProducts} userDataId={userDataId} title="Add product" onSubmit={() => showNotification('Product Added!')} />
        </div>
    )
}
export default Home
